...
class PlaceRecord extends CActiveRecord {
    ...
    public function relations() {
        ...
        return array(
            ...
            'zoneFk' => array(self::BELONGS_TO, 'Zones', 'zone_id_fk'),
        );
    }
    ...
    public static function searchPlaceForComent($coments) {
        ...
        $places = Yii::app()->db->createCommand()
                ->select('place_id')
                ->from('places')
                ->leftJoin('zones', 'places.zone_id_fk = zones.zone_id')
...
//            $places = new PlaceRecord();
//            $places = Yii::app()->db->createCommand()
//                    ->select(' zones.zone_name,place_name, place_phone, place_movil, place_coments')
//                    ->from('places')
//                    ->leftJoin('zones', 'places.zone_id_fk = zones.zone_id')
//                    ->where(array('like','place_coments','%'.$coments.'%'))
//                    ->queryAll();
...
//            return $places;
    ...
    public static function searchPlaceForKeyWord($keyWord) {
        ...
        $places = Yii::app()->db->createCommand()
                ...
                ->from('places')
                ->leftJoin('zones', 'places.zone_id_fk = zones.zone_id')
                ->where(array('like', 'key_words', '%' . $keyWord . '%'))
                ->queryAll();
...
//            return $places;
     ...
     * 
     */
    public static function searchPlacesByPlaceId($place_id) {
        $places = new PlaceRecord();
        $places = Yii::app()->db->createCommand()
                ->select(' zones.zone_name,place_name, place_phone, place_movil, place_coments')
                ->from('places')
                ->leftJoin('zones', 'places.zone_id_fk = zones.zone_id')
                ->where('place_id = :place_id', array('place_id'=>$place_id))
                ->queryRow();
     ...
     * 
     */
    public static function searchPlacesById($place_id) {
        $places = new PlaceRecord();
        $places = Yii::app()->db->createCommand()
                ->select(' zones.zone_name,place_name, place_phone, place_movil, place_coments')
                ->from('places')
                ->leftJoin('zones', 'places.zone_id_fk = zones.zone_id')
                ->where('id = :id', array('id'=>$place_id))
                ->queryRow();
